...
class ZSpotify:
    ...
    def parse_args(self):
        ...
        parser.add_argument("--limit", help="limit", default=_LIMIT_RESULTS, type=int)
        parser.add_argument(
            "-f",
            "--force-premium",
    ...
    def set_audio_tags(
        ...
        if extension == "mp3":
            ...
            if name is not None:
                ...
                tags["TIT2"] = id3.TIT2(encoding=3, text=name)
            ...
            if track_number is not None:
                ...
                tags["TRCK"] = id3.TRCK(encoding=3, text=str(track_number))
    ...
    def generate_filename(
        ...
        def shorten_filename(filename, audio_name, artist_name, max_length=255):
        ...
        if caller == "album":
            ...
            base_path = path or self.music_dir
            filename = f"{artist_name}.{audio_format}"

            if self.album_in_filename:
                filename = f"{album_name} - " + filename
            filename = f"{audio_name} - " + filename
            ...
            base_path = path or self.music_dir
            filename = f"{audio_name} - {artist_name} .{audio_format}"

        filename = shorten_filename(filename, audio_name, artist_name)
    ...
    def download_track(self, track_id, path=None, caller=None):
        ...
        if self.not_skip_existing and fullpath.exists():
            ...
            return True
        downloader = Thread(
            target=self.zs_api.download_audio, args=(track_id, fullpath, True)
        )
        downloader.start()
    ...
    def download_episode(self, episode_id, caller="episode"):
        ...
        downloader = Thread(
            target=self.zs_api.download_audio, args=(episode_id, fullpath, True)
        )
        downloader.start()
